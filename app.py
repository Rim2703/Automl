...
if choice == "Profiling":
    ...
    # Generate and display correlation report as PDF
    st.title("Phi_k Correlation Report")
    report.correlation_report(df, pdf_file_name='phik_correlation_report.pdf')
    with open('phik_correlation_report.pdf', 'rb') as f:
        st.download_button('Download Correlation Report as PDF', f, file_name='phik_correlation_report.pdf')
...
if choice == "ML":
    ...
    if st.button("Train model"):
        ...
        best_model = compare_models()
        compare_df = pull()
        st.info("This is the Automl model comparison")
        st.dataframe(compare_df)
        best_model
        save_model(best_model, "best_model")
